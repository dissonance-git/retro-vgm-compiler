...
int main() {
    ...
    const auto trace = begin_vgm_execution_trace(graph, "fixture.vgm", capture_flags);
    ...
    const auto ym_event_id = append_vgm_trace_event(
        graph,
        trace,
        command_event{command_event_kind::command, 20, 0x110, 0x52, ym, 2});
    ...
    CHECK(ym_event->active->start.tick == 20);
