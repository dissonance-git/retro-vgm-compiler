...
int main() {
    ...
    // Reusing one hardware slot is only a weak continuity cue. It must never
    // become a strong musical-part identity by itself.
    ...
    // Same source/patch identity is meaningful, but one identity domain alone
    // still stops below the strong-part threshold.
    ...
    // Identity plus independent temporal/pitch continuity is enough to preserve
    // a strong runtime-derived hypothesis without claiming an authored track.
    ...
    // Strong simultaneous/conflicting evidence forces the runtime hypothesis
    // back below coin-flip confidence unless documentary identity overrides it.
    ...
    // Explicit driver identity is a different epistemic situation and may
    // ground the part despite lower-level ambiguity.
