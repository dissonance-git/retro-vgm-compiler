...
tertian_triad_hypothesis chord(
    ...
    result.projection.source_verticality.observation_time = at(tick);
    return result;
}
...
int main() {
    ...
    CHECK(moving_bass.first_time.tick == 0);
    CHECK(moving_bass.second_time.tick == 100);
    ...
    // Voice-leading from another transition cannot be loaned into this bass
    // relation even when its pitches and part IDs happen to look compatible.
    bool mismatched_transition_rejected = false;
    try {
        const auto stale_voices = voices({
            {48, 45, -3, 1, 1, true},
            {60, 60, 0, 2, 2, true},
            {64, 64, 0, 3, 3, true},
        }, 0.99, 10, 110);
        (void)infer_bass_harmony_interaction(c_major, a_minor, stale_voices);
    } catch (const std::invalid_argument&) {
        mismatched_transition_rejected = true;
    }
    CHECK(mismatched_transition_rejected);
