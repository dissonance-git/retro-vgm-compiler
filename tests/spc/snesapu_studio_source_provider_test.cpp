...
int main() {
    ...
    assert(!provider.begin_voice(0, 7, 0x4201, 0));
    assert(!provider.voice_active(0));
    assert(provider.begin_voice(0, 7, 0x4200, 0));
    assert(provider.voice_active(0));
    ...
    for (const std::uint32_t rate : rates) {
        ...
        float rendered = 0.0f;
        assert(provider.render_voice(0, rate, 0, &rendered));
    ...
    assert(expected.valid);
    float rendered = 0.0f;
    assert(provider.render_voice(0, 0x00010000u, 3, &rendered));
    assert(near(rendered, expected.sample));
    ...
    assert(provider.begin_voice(1, 7, 0x4200, 1));
    rendered = 123.0f;
    assert(provider.render_voice(1, 0x00010000u, 1, &rendered));
    ...
    // An impossible rate is a permanent loss of phase authority for this key-on,
    // so Enhanced substitution deactivates rather than drifting from SNESAPU.
    assert(!provider.render_voice(1, 0x01000000u, 1, &rendered));
    ...
    assert(snesapu_studio_source_provider<4>::begin_callback(
        nullptr, 0, 7, 0x4200, 0) == 0u);
    assert(snesapu_studio_source_provider<4>::sample_callback(
        nullptr, 0, 0x00010000u, 0, &rendered) == 0u);
