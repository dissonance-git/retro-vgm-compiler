...
OpenMusic and its library ecosystem are research observatories for VGM Compiler. They are not runtime dependencies and do not define the project ontology. Detailed repository and literature notes live in `research/cases/openmusic-libraries.md`.
...
The OpenMusic pass now supports the upper musical-analysis and reconstruction layers rather than defining the immediate frontier. VGM Compiler already has executable regressions for competing persistent-part hypotheses, source-relative feature availability, theory and perceptual alternatives, listener-response context, musicological identity, timbre versus instrument identity, role-relative attribution, and programmed expression versus higher interpretation.
