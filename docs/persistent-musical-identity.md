...
This document records the current VGM Compiler frontier for recovering persistent musical voices/parts from executable game-music evidence.
...
Some symbolic voice-separation literature defines a musical voice in perceptual terms, close to an auditory stream. That is useful research but is not identical to VGM Compiler's `part` object.

VGM Compiler already has separate semantic layers and node kinds for:
...
Its cubic implementation and score-centric feature set make it a research/reference algorithm rather than a realtime VGM Compiler dependency.
...
Useful lesson for VGM Compiler:
...
The important transfer is the **relation-first** formulation. VGM Compiler does not need to import the graph neural network or its training stack to benefit from that decomposition.
...
## Why VGM Compiler can do better than score-only separation

Generic symbolic systems usually begin with note-like events because that is the highest layer they possess. VGM Compiler can often retain deeper causal evidence.
