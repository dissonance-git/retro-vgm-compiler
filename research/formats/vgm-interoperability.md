...
VGM Compiler is trying to solve a larger problem:
...
2. **A register-frequency estimate is not always the musical fundamental.**
   Later maintainer discussion notes that FM operator multipliers and modulation can make two identical nominal frequency settings sound at different effective octaves. This is exactly why VGM Compiler keeps device-native pitch, nominal device frequency, programmed pitch, performed pitch and heard pitch as separate claims.
...
VGM
 ├─> vgm2mid output
 └─> VGM Compiler symbolic projection
```
...
Disagreement is not automatically a VGM Compiler failure. It becomes an experiment:
...
### Phase 2: VGM Compiler becomes the richer semantic engine
