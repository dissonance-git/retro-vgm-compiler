...
class genesis_state {
public:
    using event_tap = void (*)(void* user, const command_event& event) noexcept;

    void reset() noexcept;
    void observe(const command_event& event) noexcept;
    void set_event_tap(event_tap tap, void* user) noexcept {
        event_tap_ = tap;
...
private:
    ...
    event_tap event_tap_ = nullptr;
    void* event_tap_user_ = nullptr;
};
