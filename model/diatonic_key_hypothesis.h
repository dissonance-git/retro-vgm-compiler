...
inline const char* to_string(diatonic_mode mode) noexcept {
    switch (mode) {
    case diatonic_mode::ionian:
        return "ionian";
    case diatonic_mode::dorian:
        return "dorian";
    case diatonic_mode::phrygian:
        return "phrygian";
    case diatonic_mode::lydian:
        return "lydian";
    case diatonic_mode::mixolydian:
        return "mixolydian";
    case diatonic_mode::aeolian:
        return "aeolian";
    case diatonic_mode::locrian:
        return "locrian";
    }
    return "unknown";
}
...
inline void validate_pitch_class_collection_profile(
    const pitch_class_collection_profile& profile) {
...
inline std::vector<diatonic_mode_candidate> infer_diatonic_mode_candidates(
    ...
    for (double value : collection.salience)
        total_salience += value;

    std::vector<diatonic_mode_candidate> candidates;
    candidates.reserve(7);
    for (diatonic_mode mode : {
             diatonic_mode::ionian,
             diatonic_mode::dorian,
             diatonic_mode::phrygian,
             diatonic_mode::lydian,
             diatonic_mode::mixolydian,
             diatonic_mode::aeolian,
        ...
        for (std::size_t pitch_class = 0; pitch_class < collection.salience.size(); ++pitch_class) {
            if (member[pitch_class]) {
                ...
                if (collection.salience[pitch_class] > 0.0)
                    ++covered_template_classes;
            }
        }
...
inline tonal_key_class_hypothesis infer_tonal_key_class_hypothesis(
    ...
    result.runner_up_score = result.alternatives.size() >= 2
        ? result.alternatives[1].composite_score
        : 0.0;
    ...
    const bool enough_center =
        center.cross_origin_grounded &&
        center.confidence >= diatonic_key_min_center_confidence;
    const bool enough_collection =
    ...
    const bool enough_fit =
        best.in_collection_fit >= diatonic_key_min_fit &&
    ...
    // Pitch class + mode still does not determine orthographic spelling or
    // common-practice harmonic function. Those remain later theory-scoped
    // hypotheses.
...
inline node_id add_tonal_key_class_hypothesis(
    ...
    key.attributes.push_back({
        "identity_scope",
        std::string{"tonal_key_class_hypothesis"},
    ...
    key.attributes.push_back({
        "mode",
        std::string{to_string(*hypothesis.mode)},
        evidence_status::hypothesis,
        hypothesis.confidence,
        "",
    });
    ...
    key.provenance.push_back({
        evidence_status::hypothesis,
        hypothesis.confidence,
        "tonal center + pitch-class collection",
        std::nullopt,
