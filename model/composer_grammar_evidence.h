...
inline composer_grammar_rule make_composer_grammar_rule(
    ...
    for (const auto& observation : observations) {
        ...
        if (observation.polarity == composer_grammar_polarity::supports) {
            ...
            ++support_count;
            work_families.insert({observation.soundtrack_id, observation.work_family_id});
    ...
    result.cross_work_grounded = work_families.size() >= 2;
    result.cross_soundtrack_grounded = soundtracks.size() >= 2;
